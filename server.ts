...
async function startServer() {
  ...
  // Configure uploads folder static middleware
  ...
  try {
    ...
    console.warn('Could not create uploads directory (expected in read-only serverless environments):', err);
  }
  ...
  app.get('/api/analytics/overview', checkRole(['ADMIN', 'JOURNALIST']), async (req, res) => {
    try {
      ...
      const users = await db.getAllUsers();
      // Count subscribers as regular users now that paywalls are gone, or count role 'SUBSCRIBER'
